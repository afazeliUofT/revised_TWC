...
def main():
    ...
    ssh(f"cd {REMOTE_ROOT} && source .venv/bin/activate && python scripts/prepare_github_artifacts.py", check=False)
    dest = local_root / "results" / "from_rorqual"
    dest.mkdir(parents=True, exist_ok=True)
    run(["rsync", "-av", "--delete", f"{REMOTE}:{REMOTE_ROOT}/outputs/github_artifacts/", str(dest) + "/"])
    git_commit_push(local_root, "sync compact Rorqual results")
    print("DONE: compact results synced and pushed if GitHub SSH is configured.")
